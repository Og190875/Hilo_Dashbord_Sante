...
def index():
    ...
    # Si module poids actif + API Withings → afficher le dernier poids Withings
    if modules.get('poids', {}).get('actif') and modules.get('poids', {}).get('poids_api'):
        try:
            import sqlite3 as _sq
            with hilo_db.get_conn(db_path) as _conn:
                row_pw = _conn.execute(
                    "SELECT poids FROM poids_historique WHERE poids IS NOT NULL ORDER BY mesure_le DESC LIMIT 1"
                ).fetchone()
            if row_pw:
                profil = dict(profil)
                profil['poids'] = row_pw['poids']
        except Exception:
            pass
...
def api_sql_query():
    ...
    try:
        ...
        export_all = data.get("export", False)
        rows = cur.fetchall() if export_all else cur.fetchmany(1000)  # max 1000 lignes pour affichage
...
def dashboard():
    ...
    # Sessions PPC pour filtre Zz (seulement si module sommeil actif)
    ppc_sessions_dash = []
    try:
        _mods = hilo_db.get_modules(db_path) or {}
        if _mods.get('sommeil', {}).get('actif', True):
...
def _collect_export_data(db_path):
    ...
    try:
        ...
        with hilo_db.get_conn(db_path) as conn:
            ...
            tension_rows = conn.execute(
                ...
                "ROUND(AVG(CASE WHEN heartrate > 0 THEN heartrate END),1) AS fc "
                "FROM mesures_hilo WHERE systolic IS NOT NULL GROUP BY jour ORDER BY jour"
            ).fetchall()
    ...
    try:
        ...
        for sess in sessions_ppc:
            cur_ppc.execute("""
                ...
                FROM mesures_hilo WHERE systolic IS NOT NULL
    ...
    try:
        ...
        for sv in sorties_v:
            ...
            cur2.execute("SELECT AVG(systolic) as sys, AVG(diastolic) as dia FROM mesures_hilo WHERE DATE(timestamp)=?", (sv['date'],))
            tj  = cur2.fetchone()
            cur2.execute("SELECT AVG(systolic) as sys FROM mesures_hilo WHERE DATE(timestamp)=?", (dt_j1,))
            tj1 = cur2.fetchone()
            cur2.execute("SELECT AVG(systolic) as sys FROM mesures_hilo WHERE DATE(timestamp)=?", (dt_j2,))
            tj2 = cur2.fetchone()
...
def api_am_rapport(pid):
    ...
    try:
        ...
        try:
            ...
            pass
        # Poids dynamique depuis poids_historique si module poids actif
        _mods = hilo_db.get_modules(db_path) or {}
        if _mods.get('poids', {}).get('actif', False) and proto.get('date_debut'):
            try:
                with hilo_db.get_conn(db_path) as conn:
                    row_p = conn.execute("""
                        SELECT ROUND(AVG(poids),1) as moy_poids
                        FROM poids_historique
                        WHERE DATE(mesure_le) BETWEEN DATE(?,'-30 days') AND ?
                          AND poids IS NOT NULL AND poids > 0
                    """, (proto['date_debut'], proto['date_debut'])).fetchone()
                if row_p and row_p['moy_poids']:
                    profil = dict(profil)  # copie pour ne pas modifier l'original
                    profil['poids'] = row_p['moy_poids']
            except Exception:
                pass
...
def api_correlations_velo():
    ...
    try:
        ...
        cur.execute("SELECT AVG(systolic) as sys, AVG(diastolic) as dia FROM mesures_hilo")
        ...
        for s in sorties:
            ...
            cur.execute("""
                ...
                FROM mesures_hilo WHERE DATE(timestamp) = ?
            """, (dt_cible,))
            t = cur.fetchone()

            # Tension J+1 et J+2
            cur.execute("SELECT AVG(systolic) as sys FROM mesures_hilo WHERE DATE(timestamp) = ?", (dt_j1,))
            t_j1 = cur.fetchone()
            cur.execute("SELECT AVG(systolic) as sys FROM mesures_hilo WHERE DATE(timestamp) = ?", (dt_j2,))
            t_j2 = cur.fetchone()
    ...
    try:
        with hilo_db.get_conn(db_path) as conn:
            ...
            tension_rows = conn.execute(
                ...
                f"ROUND(AVG(CASE WHEN heartrate > 0 THEN heartrate END),1) AS fc "
                f"FROM mesures_hilo WHERE systolic IS NOT NULL {t_where} "
...
def api_gps_import():
    ...
    for sid, date, debut_ts, fin_ts, duree_sec in sorties:
        ...
        n_sorties += 1
        # Inclure seulement les sorties sans météo existante
        has_meteo = bool(cur.execute("SELECT meteo_temp FROM workouts WHERE id=?", (sid,)).fetchone()[0])
        sorties_info.append({"id": sid, "date": date, "has_meteo": has_meteo})
...
def _sync_meteo_velo(db_path):
    ...
    for s in sorties:
        ...
        try:
            ...
            # Moyenne circulaire direction vent (évite erreur N/S autour de 0°/360°)
            if d_:
                import math as _mdir
                _sin = sum(_mdir.sin(_mdir.radians(d)) for d in d_)
                _cos = sum(_mdir.cos(_mdir.radians(d)) for d in d_)
                moy_dir = round(_mdir.degrees(_mdir.atan2(_sin, _cos)) % 360, 1)
            else:
                moy_dir = None
            cur.execute("""
                UPDATE workouts SET
