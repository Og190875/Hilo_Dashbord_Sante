...
def render_rapport(proto, grille, profil, traitements=None, mesures_base=None, proto_prec=None, grille_prec=None):
    ...
    # Calcul moyenne Suivi continu (bracelet Hilo) depuis mesures_base pour le résumé final
    moy_sys_base = moy_dia_base = gc_base = None
    if mesures_base:
        _bsys = [v['sys'] for v in mesures_base.values() if v.get('sys')]
        _bdia = [v['dia'] for v in mesures_base.values() if v.get('dia')]
        if _bsys:
            moy_sys_base = round(sum(_bsys)/len(_bsys), 1)
            moy_dia_base = round(sum(_bdia)/len(_bdia), 1)
            gc_base, _ = _cc(moy_sys_base, moy_dia_base, classif, profil)
    ...
    synthese_html = (
        ...
        '</td>'
        + (f'<td style="padding:10px 16px;background:{glight};text-align:center;border-left:1px solid #e2e8f0"><div style="font-size:.7rem;color:#94a3b8;margin-bottom:4px">Suivi continu</div><div style="font-size:1.1rem;font-weight:700;color:{gc_base or "#1e3a5f"}">{moy_sys_base}&nbsp;/&nbsp;{moy_dia_base}</div><div style="font-size:.62rem;color:#94a3b8">mmHg</div></td>' if moy_sys_base and moy_dia_base else '<td style="background:{glight}"></td>') +
        '</tr></table></div>'
    )
