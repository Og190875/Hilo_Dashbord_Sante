...
def migrate_db(conn):
    ...
    # V9.0.8 : vue mesures_hilo (exclut les mesures d'automesure des stats/courbes)
    try:
        conn.execute("""
            CREATE VIEW IF NOT EXISTS mesures_hilo AS
                SELECT * FROM mesures
                WHERE source_pdf IS NULL OR source_pdf != 'AUTOMESURE'
        """)
    except Exception:
        pass
...
def init_db(db_path):
    ...
    conn0.executescript("""
            ...
            -- Vue excluant les mesures d'automesure (source_pdf = 'AUTOMESURE')
            -- Utilisée partout sauf dans l'onglet Automesures
            CREATE VIEW IF NOT EXISTS mesures_hilo AS
                SELECT * FROM mesures
                WHERE source_pdf IS NULL OR source_pdf != 'AUTOMESURE';
...
def tag_bras_s_automesure(db_path):
    """
    Tague automatiquement en AUTOMESURE les mesures du bracelet Hilo
    qui correspondent à des séances bras='S' des protocoles d'automesure.
    Matching : même sys/dia exact + diff <= 10 minutes.
    Appelé après chaque import PDF et après chaque injection de protocole.
    """
    DIFF_MAX_SEC = 10 * 60  # 10 minutes
    try:
        with get_conn(db_path) as conn:
            seances_s = conn.execute("""
            ...
            meilleurs = {}  # id_mesure → diff_sec minimum
            for s in seances_s:
                ts   = s['timestamp']
                date = ts[:10]
                rows = conn.execute("""
                    SELECT id,
                           ABS(CAST(strftime('%s', timestamp) AS INTEGER) -
                               CAST(strftime('%s', REPLACE(?, ' ', 'T')) AS INTEGER)) as diff_sec
                    FROM mesures
                    WHERE DATE(timestamp) = ?
                      AND systolic  = ?
                      AND diastolic = ?
                      AND (source_pdf IS NULL OR source_pdf != 'AUTOMESURE')
                      AND ABS(CAST(strftime('%s', timestamp) AS INTEGER) -
                              CAST(strftime('%s', REPLACE(?, ' ', 'T')) AS INTEGER)) <= ?
                    ORDER BY diff_sec
                    LIMIT 1
                """, (ts, date, s['systolic'], s['diastolic'], ts, DIFF_MAX_SEC)).fetchall()

                if rows:
                    mid  = rows[0]['id']
                    diff = rows[0]['diff_sec']
                    if mid not in meilleurs or diff < meilleurs[mid]:
                        meilleurs[mid] = diff

            # Appliquer le tag
            for mid in meilleurs:
                conn.execute(
                    "UPDATE mesures SET source_pdf='AUTOMESURE' WHERE id=?", (mid,)
                )
        return len(meilleurs)
    except Exception as e:
        print(f"[tag_bras_s_automesure] Erreur : {e}")
        return 0
...
def inject_mesures(db_path, df, source_pdf=""):
    ...
    try:
        ...
        # Matching automatique bras S → AUTOMESURE
        tag_bras_s_automesure(db_path)
...
def get_mesures(db_path, date_start=None, date_end=None):
    """Retourne les mesures sous forme de liste de dicts (hors automesures)."""
    sql  = "SELECT * FROM mesures_hilo"
    args = []
    conditions = []
    if date_start:
